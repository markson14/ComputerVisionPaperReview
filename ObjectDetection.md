...
Class RPN():
	def __init__(self):
		RPN_conv = nn.conv2d(in,in,(3,3))
		RPN_cls_score = nn.conv2d(in,2*k,(1,1))
		RPN_bbox_pred = nn.conv2d(in,4*k,(1,1))
...
- 对RoI mapping进行了整数量化操作。没有align

#### 三者对比：

![Screen Shot 2020-06-05 at 5.40.20 pm](assets/Screen%20Shot%202020-06-05%20at%205.40.20%20pm.png)
...
for ROI in ROIs:
    ...
    class_scores, box = average(V)                 #much simple!
    class_probabilities = softmax(class_scores)
```
...
- Pooling对于第(I, J)个bin的操作，c为第c-th个class：是其中的一个通道，是一个里数量
...
1. Smooth-L1：解决了收敛问题；可是由于xywh全部单独回归，不符合现实。
2. IoU：解决了单独回归的问题；可是在gt和predict不相交的情况下Loss为0，不可导。无法反映两框真实相交情况。$IoU \ Loss = 1 - IoU$
3. GIoU：用最小外接矩形解决了不想交不可导情况下的问题；当gt框包含预测框时会退化成IoU Loss，无法获取相对位置关系。$GIoU \ Loss = 1 - GIoU)$
4. DIoU：用中心点距离解决了退化问题，加速收敛。$DIoU \ Loss = 1 - IoU + centerdistance$
5. CIoU：在DIoU上增加一个惩罚项，吧预测框的长宽比你和目标框长宽比考虑进去。$IoU \ Loss = 1 - IoU + centerdistance + \alpha v$
